...
test(`Should open modal when clicking open modal button, then add row to table when clicking add move button`, async ({ page }) => {
...
test(`Should open modal when pressing open modal hotkey, then add move row to table when pressing confirm hotkey`, async ({ page }) => {
	await expect(page.getByTestId("move-row")).not.toBeVisible();
